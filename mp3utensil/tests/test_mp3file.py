...
class Test_MP3File(unittest.TestCase):
    ...
    def test_short_numpy_read_header(self):
        ...
        byte_array = mp3file.NumpyArrays(self.temp)
        self.read_header(byte_array)
        
    def test_short_python_read_header(self):
    ...
    def read_header(self, byte_array):
        ...
        h2 = byte_array.read_header_struct(208)
    ...
    def test_short_numpy_generate_potential(self):
        """Testing of the numpy array's potential generator"""
        array = mp3file.NumpyArrays(self.temp)
        self.potential_generator(array)
        
    def test_short_python_generate_potential(self):
        """Testing of the python array's potential generator"""
        array = mp3file.PythonArrays(self.temp)
        self.potential_generator(array)
        
    def potential_generator(self, array):
        """Tests the potential header generator for a given array"""
        possibles = []
        for p in array.generate_potential_h_structs(-1):
            possibles.append(p[0])
        self.assertEquals(possibles[0], 0, 
                          "Potential header not identified in sample file")
        self.assertEquals(possibles[1], 48, 
                          "Potential header not identified in sample file")
        self.assertEquals(possibles[2], 56, 
                          "Potential header not identified in sample file")
        
        possibles = []
        for p in array.generate_potential_h_structs(56):
            possibles.append(p[0])
        self.assertEquals(possibles[0], 65,
                    "Potential header not identified in middle of sample file")
        self.assertEquals(possibles[1], 73,
                    "Potential header not identified in middle of sample file")
        self.assertEquals(possibles[2], 74,
                    "Potential header not identified in middle of sample file")
    
    @unittest.skipIf(not mp3file.NUMPY_AVAILABLE, "Numpy not available to test")    
    def test_short_numpy_array_lockon(self):
    ...
    def test_short_no_numpy(self):
        ...
        mfile = mp3file.MP3File(self.temp)
        self.assertEquals(mfile._array_type, mp3file.PythonArrays)

    @unittest.skipIf(not mp3file.NUMPY_AVAILABLE, "Numpy not available to test")    
    def test_short_numpy_scan_file(self):
        """Testing MP3File's file_scan method"""
        self.temp.close()
        mfile = mp3file.MP3File(self.fname)
        mfile.scan_file()
        temp = []
